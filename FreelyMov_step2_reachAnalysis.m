...
for m = 1:num_animals
    ...
    for s=1:num_sessions
        ...
        % Find the IRIs that were detetced in trials
        trial_start = [0; cumsum(trials.size_trial(1:end-1))];  % Offset for each trial
        reach_max_sess_idx = trial_start(reaches.reach_trial) + ...
            reaches.reach_range_mat(:,reaches.reach_params.reach_interval.max_reach);
        [idx_match_iri, idx_in_reach] = ismember(sessions.pks_frames, reach_max_sess_idx);
        pks_matching = sessions.pks_frames(idx_match_iri);

        % IRI
        pks_time_sec = pks_matching/sessions.video.frame_rate;
        iri_sec = [0;diff(pks_time_sec)];

        % Find the x poisiton at start
        reach_xz_at_start = nan(2,size(reaches.reach_mat,3));
        for i = 1:size(reaches.reach_mat,3)
            reach_xz_at_start(:,i) = reaches.reach_mat(reaches.start_forw.start_forw_mat(i),:,i);
        end
